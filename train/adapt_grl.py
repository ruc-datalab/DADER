...
def train(args, encoder, classifier, dom_classifier, src_data_loader, tgt_data_train_loader, tgt_data_valid_loader):
    ...
    optimizer = optim.Adam(list(encoder.parameters())+list(classifier.parameters())+list(dom_classifier.parameters()), lr=param.d_learning_rate)
    ...
    for epoch in range(args.num_epochs):
        ...
        if len(src_data_loader)>len(tgt_data_train_loader):
            data_zip = enumerate(itertools.zip_longest(src_data_loader, tgt_data_train_loader))
        else:
        ...
        for step, (src, tgt) in data_zip:
            if tgt:
                ...
                label_tgt = make_cuda(torch.zeros(reviews_tgt.size(0))).unsqueeze(1)
                label_concat = torch.cat((label_src, label_tgt), 0).float()
                loss_dom = BCELoss(dom_preds, label_concat)
                if args.source_only:
                    loss = cls_loss
                else:
                    loss = cls_loss + args.beta*loss_dom
            else:
        ...
        f1_train,eloss_train = evaluate(encoder, classifier, tgt_data_train_loader, epoch=epoch, pattern=1000)
        f1_valid,eloss_valid = evaluate(encoder, classifier, tgt_data_valid_loader, epoch=epoch, pattern=1000)
        
        if f1_valid>bestf1:
            save_model(args, encoder, param.src_encoder_path+'grlbestmodel')
            save_model(args, classifier, param.src_classifier_path+'grlbestmodel')
            bestf1 = f1_valid
...
def evaluate(encoder, classifier, data_loader,flag=None,epoch=None,pattern=None):
