...
def train(args, encoder, classifier, src_data_loader, tgt_data_train_loader, tgt_data_valid_loader):
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, (src, tgt) in data_zip:
            if tgt:
                ...
                if args.source_only:
                    ...
                    loss = cls_loss + args.beta * loss_coral
            else:
            ...
            loss.backward()
            optimizer.step()
            korder_sum += loss_coral.item()
            if (step + 1) % args.log_step == 0:
                print("Epoch [%.2d/%.2d] Step [%.3d/%.3d]: "
                      "k-order_loss=%.4f cls_loss=%.4f js=%.4f"
                      % (epoch + 1,
                         args.num_epochs,
                         step + 1,
                         len_data_loader,
        ...
        if f1_valid>bestf1:
            save_model(args, encoder, param.src_encoder_path+'korderbestmodel')
            save_model(args, classifier, param.src_classifier_path+'korderbestmodel')
            bestf1 = f1_valid
