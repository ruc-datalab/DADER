...
def parse_arguments():
    ...
    parser.add_argument('--src', type=str, default="books",help="Specify src dataset")

    parser.add_argument('--tgt', type=str, default="dvd",help="Specify tgt dataset")
...
def main():
    ...
    if args.src not in ["kitchen","electronics","dvd","books"]:
    ...
    src_x_p1, src_test_x, src_y_p1, src_test_y = train_test_split(src_x, src_y,
                                                            test_size=0.2,
                                                            stratify=src_y,
    ...
    src_data_eval_loader = get_data_loader(src_test_features, args.batch_size,"dev")
    tgt_data_train_loader = get_data_loader(tgt_train_features, args.batch_size,"train")
    # tgt_data_all_loader = get_data_loader(tgt_features, args.batch_size,"dev")
    tgt_data_all_loader = get_data_loader(tgt_train_features, args.batch_size,"dev")
    ...
    if args.load:
        src_encoder = init_model(args, src_encoder, restore=param.src_encoder_path+'epoch'+str(3))
        src_classifier = init_model(args, src_classifier, restore=param.src_classifier_path+'epoch'+str(3))
        tgt_encoder = init_model(args, tgt_encoder, restore=param.tgt_encoder_path+'epoch'+str(3))
        discriminator = init_model(args, discriminator, restore=param.d_model_path+'epoch'+str(3))
    else:
    ...
    if args.pretrain:
        src_encoder, src_classifier = pretrain(
            args, src_encoder, src_classifier, src_data_loader, tgt_data_all_loader)
    ...
    if args.adapt:
        ...
        tgt_encoder,discriminator = adapt(args, src_encoder, tgt_encoder, discriminator,
