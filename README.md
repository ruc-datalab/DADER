...
> * For the Feature Aligner methods, discrepancy-based methods (e.g., MMD) are more stable but adversarial-based method InvGAN+KD works the best when carefully tuning hyper-parameters.

The following is the convergence comparison of MMD and InvGAN+KD. Each figure shows the changes of F1 score with epochs. The red line corresponds to InvGAN+KD, the blue line corresponds to MMD, and the green line corresponds to NoDA.

<img src="figure/convergence.png" width="700" />

> * When the labels for target datasets are available, using DA requires much less training data from the target to achieve a high accuracy, comparing with the SOTA DL based methods with/without pre-trained languages models.

<img src="figure/semi.png" width="700" />

> * DA can significantly improve ER when the model trained on the source does not perform well on the target.
> * Choosing a “close” source domain for DA is more beneficial.
> * Not all successful DA methods in CV and NLP are suitable to ER.
> * Pre-trained LM Feature Extractor works better than RNN based methods.
> * the feature-level DA approach that learns domain-invariant and discriminative features is much better than the instance-based approach.

## Quick Start
Note that this is the experimental version of the code, the main purpose is to help the reader reproduce the experimental results.
...
Note that we have adjusted the parameters according to the validation set. If necessary, you can adjust the parameters in the script as described in the paper to observe the changes in the experimental results. For example, learning rate, parameter ` beta`, etc.
